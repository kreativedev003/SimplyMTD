...
namespace SimplyMTD.Pages
{
	public partial class Index
	{
		...
		protected async System.Threading.Tasks.Task Load()
		{
			...
			{
				all_clients = await VATService.GetClientsForAgent();
		...
		{
			await DialogService.OpenAsync<AddClient>("Add Client", new Dictionary<string, object> {  }, new DialogOptions() { Width = "1200px", Height = "700px" });
			await Load();
		}

		protected async Task OnDetail(Radzen.DataGridRowMouseEventArgs<UserDetail> item)
		{
			await DialogService.OpenAsync<AddClient>("Edit Client", new Dictionary<string, object> { { "client", item.Data } }, new DialogOptions() { Width = "1200px", Height = "700px" });
            await Load();
        }
