...
namespace SimplyMTD
{
    public partial class SecurityService
    {
        ...
        public MTDContext _MTDContext { get; private set; }
        ...
        public async Task<bool> InitializeAsync(AuthenticationState result)
        {
            ...
            if (userId != null && User?.Id != userId)
            {
                ...
                userDetail = this._MTDContext.UserDetails.Where(user => user.UserId == userId).First();
            }
        ...
        public async Task Register(string userName, string password, string email, int currentTab)
        {
            ...
            var content = new FormUrlEncodedContent(new Dictionary<string, string> {
                ...
                { "password", password },
				{ "email", email },
				{ "accountType", currentTab.ToString() },
			});
