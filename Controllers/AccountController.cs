...
namespace SimplyMTD.Controllers
{
    ...
    public partial class AccountController : Controller
    {
        ...
        private readonly MTDContext _MTDContext;
        ...
        public async Task<IActionResult> Login(string userName, string password, string redirectUrl)
        {
            ...
            if (!string.IsNullOrEmpty(userName) && !string.IsNullOrEmpty(password))
            {
                ...
                if (result.Succeeded)
                {
                    var userDetail = _MTDContext.UserDetails.Where(us => us.UserId == user.Id).First();
                    if(!userDetail.Token.IsNullOrEmpty())
                    {
						_tokenProvider.AccessToken = userDetail.Token;
        ...
        [HttpPost]
        public async Task<IActionResult> Register(string userName, string password, string email, string accountType)
        {
            if (string.IsNullOrEmpty(userName) || string.IsNullOrEmpty(password))
            {
            ...
            var clientId = ""+ Convert.ToInt32(seconds % 100);
            ...
            if (result.Succeeded)
            {
                try
                {
                    ...
                    var userDetail = new UserDetail { UserId = user.Id, ClientId =  clientId, Type = int.Parse(accountType), OwnerName = userName, Email = email};
        ...
        public async Task<IActionResult> UserRestrictedCall(string key, string vrn)
        {
            ...
            {
                UserDetail user = _MTDContext.UserDetails.First(user => user.UserId == key);
				...
				_MTDContext.UserDetails.Update(user);
				_MTDContext.SaveChanges();
			}
            ...
            {
                return Challenge(new AuthenticationProperties() { RedirectUri = $"/Account/UserRestrictedCall?key={key}&vrn={vrn}" }, "HMRC");
            }
        }

		public async Task<IActionResult> UserRestrictedCallFromGuest(string vrn)
		{
			string accessToken = await HttpContext.GetTokenAsync(IdentityConstants.ExternalScheme, "access_token");
			...
			{
				return Challenge(new AuthenticationProperties() { RedirectUri = "/Account/UserRestrictedCallFromGuest" }, "HMRC");
			}
		}
