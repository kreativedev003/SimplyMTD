...
builder.Services.AddScoped<SimplyMTD.ClientService>();
builder.Services.AddScoped<SimplyMTD.TokenProvider>();
...
Console.WriteLine(builder.Configuration.GetConnectionString("MTDConnection"));
...
}).AddCookie().AddOAuth("HMRC", options =>
{
    ...
    options.Scope.Add("read:vat write:vat read:sent-invitations write:sent-invitations write:cancel-invitations read:check-relationship");
...
builder.Services.AddScoped<SimplyMTD.VATService>(provider =>
    new SimplyMTD.VATService(
		
		provider.GetService<TokenProvider>(),
		provider.GetService<MTDContext>(),
        provider.GetService<IConfiguration>(),
		provider.GetService<SecurityService>(),
